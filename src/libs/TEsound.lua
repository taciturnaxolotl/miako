...
function TEsound.play(sound, sourceType, tags, volume, pitch, func)
    ...
    table.insert(TEsound.channels, {
        love.audio.newSource(sound, sourceType),
        func,
...
function TEsound.playLooping(sound, sourceType, tags, n, volume, pitch)
    return TEsound.play(sound, sourceType, tags, volume, pitch, (not n or n > 1) and function(d)
        TEsound.playLooping(sound, sourceType, tags, (n and n - 1), d[1], d[2])
    end)
end
...
function TEsound.volume(channel, volume)
    if type(channel) == "number" then
        ...
        volume = volume or c[3][1]
        c[3][1] = volume
        c[1]:setVolume(volume * TEsound.findVolume(c.tags) * (TEsound.volumeLevels.all or 1))
    elseif type(channel) == "string" then
        TEsound.volumeLevels[channel] = volume
        for k, v in pairs(TEsound.findTag(channel)) do
            TEsound.volume(v, volume)
        end
    end
end
...
function TEsound.cleanup()
    for k, v in ipairs(TEsound.channels) do
        if not v[1]:isPlaying() then
            if v[2] then
                v[2](v[3])
            end -- allow sounds to use custom functions (primarily for looping, but be creative!)
...
function TEsound.findTag(tag)
    ...
    for channel, sound in ipairs(TEsound.channels) do
        if sound.tags then
            for k, v in ipairs(sound.tags) do
...
function TEsound.findVolume(tag)
    if type(tag) == "string" then
        return TEsound.volumeLevels[tag] or 1
    elseif type(tag) == "table" then
        for k, v in ipairs(tag) do
            if TEsound.volumeLevels[v] then
                return TEsound.volumeLevels[v]
            end
        end
    end
    return 1
...
function TEsound.findPitch(tag)
    if type(tag) == "string" then
        return TEsound.pitchLevels[tag] or 1
    elseif type(tag) == "table" then
        for k, v in ipairs(tag) do
            if TEsound.pitchLevels[v] then
                return TEsound.pitchLevels[v]
            end
        end
    end
    return 1
